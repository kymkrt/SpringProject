...
<head>
	...
	<title>adminProductDataDetailInfo.jsp</title>
	...
	<script type="text/javascript">
  	...
  	document.addEventListener('DOMContentLoaded', function () {
		...
		function deleteCheck() {
			...
			if(ans){
				location.href="${ctp}/admin/adminProductDataDelete?idx=${vo.idx}";
			}
			else {
				return false;
			}
		...
		function fCheck() {
			...
			let images = [
			    { input: mainImage, name: "메인 이미지", checkField: "mainImageCheck" },
			    { input: detailedImage, name: "세부 이미지", checkField: "detailImageCheck" }
			];

			for (let image of images) {
			    if (image.input.files.length === 0) {
			        // 기존 이미지 사용 시
			        alert(image.name + "는 기존 이미지를 사용합니다.");
			        myform[image.checkField].value = "기존이미지사용"; // 파일 없음을 나타내는 값 설정
			    } else {
			        // 파일 선택 시
			        ...
			        if (fileSize > maxSize) {
			            ...
			            myform[image.checkField].value = "파일있음"; // 모든 이미지 검사 통과
			        }
			    }
			}
...
<div id="container">
	<h2>상품 데이터 등록</h2>
	<form id="myform" name="myform" action="${ctp}/admin/adminProductDataUpdate" method="post" enctype="multipart/form-data">
		<table class="table table-bordered table-hover align-middle">
			<tr>
				...
				<td>
					<div class="input-group">
						<input type="text" id="productName" name="productName" value="${vo.productName}" class="form-control" required />
			...
			<tr>
				...
				<td>
					<div class="d-flex justify-content-around align-items-center">
						<span><input type="radio" id="productCategory1" name="productCategory" value="식물" class="form-check-input" ${vo.productCategory=='식물' ? 'checked' : ''} /><label for="productCategory1">&nbsp;식물</label>&nbsp;</span>
						<span><input type="radio" id="productCategory2" name="productCategory" value="부자재" class="form-check-input" ${vo.productCategory=='부자재' ? 'checked' : ''} /><label for="productCategory2">&nbsp;부자재</label>&nbsp;</span>
						<span><input type="radio" id="productCategory3" name="productCategory" value="기타" class="form-check-input" ${vo.productCategory=='기타' ? 'checked' : ''} /><label for="productCategory3">&nbsp;기타</label>&nbsp;</span>
			...
			<tr>
				...
				<td>
					<div class="input-group">
						<input type="text" id="productPlantName" name="productPlantName" value="${plantVO.plantName}" class="form-control" />
			...
			<tr>
				...
				<td>
					<input type="number" id="productPrice" name="productPrice" value="${vo.productPrice}" class="form-control" required />
				</td>
			</tr>
			<tr>
				<th class="bg-dark-subtle">
				...
				<td>
					<input type="number" id="deliveryCost" name="deliveryCost" value="${vo.deliveryCost}" class="form-control" required />
				</td>
			</tr>
			<tr>
				<th class="bg-dark-subtle">
				...
				<td>
					<input type="number" id="productStock" name="productStock" value="${vo.productStock}" class="form-control" required />
				</td>
			</tr>
			<tr>
				<th class="bg-dark-subtle">
				...
				<td>
					<textarea id="productDescription" name="productDescription" rows="5" placeholder="상세설명을 적어주세요" class="form-control">${vo.productDescription}</textarea>
				</td>
			</tr>
			<tr>
				<th class="bg-dark-subtle">
				...
				<td>
					<div class="d-flex justify-content-around">
						<span><input type="radio" id="productStatus1" name="productStatus" value="판매중" class="form-check-input" ${vo.productStatus=='판매중' ? 'checked' : ''} /><label for="productStatus1">&nbsp;판매중</label>&nbsp;</span>
						<span><input type="radio" id="productStatus2" name="productStatus" value="임시등록" class="form-check-input" ${vo.productStatus=='임시등록' ? 'checked' : ''} /><label for="productStatus2">&nbsp;임시등록</label>&nbsp;</span>
						<span><input type="radio" id="productStatus3" name="productStatus" value="판매중지" class="form-check-input" ${vo.productStatus=='판매중지' ? 'checked' : ''} /><label for="productStatus3">&nbsp;판매중지</label>&nbsp;</span>
						<span><input type="radio" id="productStatus4" name="productStatus" value="재고없음" class="form-check-input" ${vo.productStatus=='재고없음' ? 'checked' : ''} /><label for="productStatus4">&nbsp;재고없음</label>&nbsp;</span>
					</div>
				</td>
			</tr>
			<tr>
				...
				<td>
					<div class="input-group">
						<input type="text" id="productTag" name="productTag" value="${vo.productTag}" class="form-control" />
					</div>
				</td>
			</tr>
			<tr>
				...
				<td>
						<input type="number" id="discountRate" name="discountRate" max="100" min="0" value="${vo.discountRate}" class="form-control" />
				</td>
			</tr>
			<tr>
				<th class="bg-dark-subtle">
				...
				<td>
						...
						<div style="width: 500px;" class="d-flex justify-content-center my-3">
	  					<img id="previewImageMain" src="${ctp}/product/${imgVO.mainImage}" style="width: 100%; height: auto;">
	  				</div>
						<div class="input-group">
						...
						<div style="width: 500px;" class="d-flex justify-content-center my-3">
	  					<img id="previewImageDetail" src="${ctp}/product/${imgVO.detailedImage}" style="width: 100%; height: auto;">
	  				</div>
		...
		<div class="d-flex justify-content-around">
			...
			<button type="button" onclick="location.href='${ctp}/admin/adminProductDataList'" class="btn btn-warning">
				돌아가기
			</button>
		...
		<input type="hidden" name="plantIdx" id="plantIdx" value="${vo.productPlantIdx}" />
		<input type="hidden" name="currentIdx" id="currentIdx" value="${vo.idx}" />
	</form>
</div>
