...
<head>
	...
	<title>adminOrderList.jsp</title>
	...
	<script type="text/javascript">
		 ...
		 function viewCheckOption(e) {//viewCheckOption_2
			    ...
			    if (!plusSelect) { // select 엘리먼트가 존재하지 않는 경우에만 생성
			        ...
			        plusSelect.addEventListener('change', (e) => {
			            ...
			            location.href = "${ctp}/admin/adminOrderList?pageSize=${pageSize}&pag=${pag}&option="+optionValue+"&subOption="+ e.target.value;
			        });
			    ...
			    $.ajax({
			        type: "post",
			        url: "${ctp}/admin/adminOrderOptionDataList",
			        data: {
			            optionData: e.value
			        },
			        dataType: "json",
		...
		$(document).ready(function() {
		    $('#chartSelect').change(function() {
		        ...
		        $.ajax({
		        	type: "post",
		        	url: "${ctp}/admin/adminOrderDataChartCountList",
		        	data: {
...
<div id="container">
	<h2 class="text-center">전체 주문 데이터 목록(관리자)</h2>
	<div class="row mb-2">
		<div class="col-4">
		</div>
		<div class="col-4"></div>
	...
	<table class="table table-hover table-bordered text-center align-middle">
		...
		<c:forEach var="vo" items="${vos}" varStatus="st">
			<tr> 
				...
				<td class="text-wrap">
					<a href="${ctp}/admin/adminProductDataDetailInfo?idx=${vo.productIdx}" data-bs-toggle="tooltip" data-bs-title="상품명:${vo.productName}<br/>상품데이터수정(관리자)" data-bs-html="true" class="link-info text-dark link-offset-2 link-opacity-75-hover link-underline-opacity-0 link-underline-opacity-100-hover">
						<b>${vo.productName}</b>
			...
			<div class="col-4">
				<div class="input-group">
					...
					<select id="chartSelect" name="chartSelect" class="form-select">
						...
						<option value="customerMid">주문자별</option>
					</select>
				</div>
			</div>
			<div class="col-4"></div>
      ...
      <form name="searchForm" method="get">
        <div class="input-group">
          ...
          <select name="part" id="part" class="form-select">
            ...
            <option value="customerMid" ${part=='customerMid'?'selected':''}>주문자</option>
          </select>
  ...
  <div class="text-center">
	  <ul class="pagination justify-content-center">
		  <c:if test="${pageVO.pag > 1}"><li class="page-item"><a class="page-link text-secondary" href="adminOrderList?pageSize=${pageVO.pageSize}&pag=1">첫페이지</a></li></c:if>
		  <c:if test="${pageVO.curBlock > 0}"><li class="page-item"><a class="page-link text-secondary" href="adminOrderList?pageSize=${pageVO.pageSize}&pag=${(pageVO.curBlock-1)*pageVO.blockSize + 1}">이전블록</a></li></c:if>
		  <c:forEach var="i" begin="${(pageVO.curBlock*pageVO.blockSize)+1}" end="${(pageVO.curBlock*pageVO.blockSize) + pageVO.blockSize}" varStatus="st">
		    <c:if test="${i <= pageVO.totPage && i == pageVO.pag}"><li class="page-item active"><a class="page-link bg-secondary border-secondary" href="adminOrderList?pageSize=${pageVO.pageSize}&pag=${i}">${i}</a></li></c:if>
		   	<!-- 배타적 처리 필요 -->
		    <c:if test="${i <= pageVO.totPage && i != pageVO.pag}"><li class="page-item"><a class="page-link text-secondary" href="adminOrderList?pageSize=${pageVO.pageSize}&pag=${i}">${i}</a></li></c:if>
		  </c:forEach>
			  <c:if test="${pageVO.curBlock < pageVO.lastBlock}"><li class="page-item"><a class="page-link text-secondary" href="adminOrderList?pageSize=${pageVO.pageSize}&pag=${(pageVO.curBlock+1)*pageVO.blockSize+1}">다음블록</a></li></c:if>
			  <c:if test="${pageVO.pag < pageVO.totPage}"><li class="page-item"><a class="page-link text-secondary" href="adminOrderList?pageSize=${pageVO.pageSize}&pag=${pageVO.totPage}">마지막페이지</a></li></c:if>
	  	</ul>
		</div>
