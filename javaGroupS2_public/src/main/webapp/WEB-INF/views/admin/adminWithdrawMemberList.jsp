...
<head>
	...
	<title>adminWithdrawMemberList.jsp</title>
	...
	//분류별 조회
	 function viewCheckOption(e, viewCheckOption_2) {
		    ...
		    if (!plusSelect) { // select 엘리먼트가 존재하지 않는 경우에만 생성
		        ...
		        plusSelect.addEventListener('change', (e) => {
		            ...
		            location.href = "${ctp}/admin/adminWithdrawMemberList?pageSize=${pageSize}&pag=${pag}&viewCheckOption=" + optionValue + "&viewCheckOption_2=" + e.target.value;
		        });
	 ...
	 document.addEventListener("DOMContentLoaded", function () {
		 ...
		 //회원 정지/탈퇴 정보 모달 열기
		    document.querySelectorAll(".open-deleteInfo-modal").forEach(button => {
	        button.addEventListener("click", function () {
	         	let modalMid = this.getAttribute("data-mid");
						$.ajax({
							type: "post",
							url: "${ctp}/admin/getAdminMemberDeleteTableByMid",
							data: {
								mid : modalMid
							},
							dataType: "json",
							success: function(res) {
								if(res !="0"){
									let deleteInfoMid = document.getElementById("deleteInfoMid");
									deleteInfoMid.value = res.deleteMid;
									let deleteInfoDate = document.getElementById("deleteInfoDate");
									deleteInfoDate.value = res.deleteDate;
									
									let deleteInfoType = document.getElementById("deleteInfoType");
									deleteInfoType.value = res.deleteType;
									
									let deleteInfoComment = document.getElementById("deleteInfoComment");
									deleteInfoComment.value = res.deleteComment;
									let deleteInfoReason = document.getElementById("deleteInfoReason");
									deleteInfoReason.value = res.deleteReason;
								}else{
									alert("회원 탈퇴/정지 정보 수정 실패");
									return false;
								}
							},error: function() {
								alert("전송오류");
	 ...
	 //회원 정지/탈퇴
		 ...
		 //회원 탈퇴/정지 정보 수정
		 function adminMemberDeleteTableUpdateByMid() {
				let deleteInfoMid = document.getElementById("deleteInfoMid").value;
				let deleteInfoType = document.getElementById("deleteInfoType").value;
				let deleteInfoReason = document.getElementById("deleteInfoReason").value;
				let deleteInfoComment = document.getElementById("deleteInfoComment").value;
								
				$.ajax({
					type: "post",
					url : "${ctp}/admin/adminMemberDeleteTableUpdateByMid",
					data: {
						deleteMid : deleteInfoMid,
						deleteType : deleteInfoType,
						deleteReason : deleteInfoReason,
						deleteComment : deleteInfoComment
					},
					success: function(res) {
						if(res != "0"){
							alert("회원의 탈퇴/정지 정보가 수정 되었습니다");
							location.reload();
						}else{
							alert("회원의 탈퇴/정지 수정 실패")
						}
					},error: function() {
						alert("전송오류");
					}
...
<div id="container">
<h2 class="text-center">탈퇴회원목록(관리자)</h2>
	<div class="row mb-2">
		<div class="col-4">
	...
	<table class="table table-hover table-bordered text-center align-middle">
		...
		<c:if test="${adminWithdrawMembers == null || fn:length(adminWithdrawMembers) == 0}">
			<tr>
				<td colspan="10">
					데이터가 없습니다.
				</td>
		...
		<c:forEach var="vo" items="${adminWithdrawMembers}" varStatus="st">
			<tr class="${vo.userDel eq 'OK' ? 'table-warning' : ''}"> 
				...
				<td>
					<c:if test="${vo.userDel=='OK'}">
						<span data-bs-toggle="tooltip" data-bs-placement="top" title="복구시키기">
				...
				<td>
					<span data-bs-toggle="tooltip" data-bs-title="회원탈퇴/정지정보">
						<button class="btn btn-info open-deleteInfo-modal"
							data-bs-toggle="modal" data-bs-target="#adminMemberDeleteInfoModal"
							data-mid="${vo.remainMid}"	>
							탈퇴정보
						</button>
					</span>
				</td>
...
<!-- 블록페이지 시작-->	
  <div class="text-center">
	  <ul class="pagination justify-content-center">
		  <c:if test="${pageVO.pag > 1}"><li class="page-item"><a class="page-link text-secondary" href="adminWithdrawMemberList?level=${level}&pageSize=${pageVO.pageSize}&pag=1">첫페이지</a></li></c:if>
		  <c:if test="${pageVO.curBlock > 0}"><li class="page-item"><a class="page-link text-secondary" href="adminWithdrawMemberList?level=${level}&pageSize=${pageVO.pageSize}&pag=${(pageVO.curBlock-1)*pageVO.blockSize + 1}">이전블록</a></li></c:if>
		  <c:forEach var="i" begin="${(pageVO.curBlock*pageVO.blockSize)+1}" end="${(pageVO.curBlock*pageVO.blockSize) + pageVO.blockSize}" varStatus="st">
		    <c:if test="${i <= pageVO.totPage && i == pageVO.pag}"><li class="page-item active"><a class="page-link bg-secondary border-secondary" href="adminWithdrawMemberList?level=${level}&pageSize=${pageVO.pageSize}&pag=${i}">${i}</a></li></c:if>
		   	<!-- 배타적 처리 필요 -->
		    <c:if test="${i <= pageVO.totPage && i != pageVO.pag}"><li class="page-item"><a class="page-link text-secondary" href="adminWithdrawMemberList?level=${level}&pageSize=${pageVO.pageSize}&pag=${i}">${i}</a></li></c:if>
		  </c:forEach>
			  <c:if test="${pageVO.curBlock < pageVO.lastBlock}"><li class="page-item"><a class="page-link text-secondary" href="adminWithdrawMemberList?level=${level}&pageSize=${pageVO.pageSize}&pag=${(pageVO.curBlock+1)*pageVO.blockSize+1}">다음블록</a></li></c:if>
			  <c:if test="${pageVO.pag < pageVO.totPage}"><li class="page-item"><a class="page-link text-secondary" href="adminWithdrawMemberList?level=${level}&pageSize=${pageVO.pageSize}&pag=${pageVO.totPage}">마지막페이지</a></li></c:if>
	  	</ul>
		</div>
		...
		<div class="modal fade" id="adminMemberDeleteModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="#adminMemberDeleteModal" aria-hidden="true">
		  <div class="modal-dialog modal-dialog-centered">
			...
			<!-- Modal 회원탈퇴정보 -->
		<div class="modal fade" id="adminMemberDeleteInfoModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="#adminMemberDeleteModal" aria-hidden="true">
		  <div class="modal-dialog modal-dialog-centered">
		    <div class="modal-content">
		      <div class="modal-header">
		        <h1 class="modal-title fs-5" id=#adminMemberDeleteInfoModal>회원 탈퇴/정지 정보 및 수정</h1>
		        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
		      </div>
		      <div class="modal-body">
		      	<form name="modalForm">
							<div class="input-group mb-2">
			      		<span class="input-group-text">아이디 : </span>
			      		<input type="text" id="deleteInfoMid" name="deleteInfoMid" class="form-control" readonly>
							</div>
							<div class="input-group mb-2">
								<span class="input-group-text">탈퇴/정지 시간 : </span>
			      		<input type="datetime" id="deleteInfoDate" name="deleteInfoDate" class="form-control" readonly>
							</div>
			      	<div class="input-group mb-2">
			      		<div class="input-group-text">
			      			종류선택
			      		</div>
				      	<select id="deleteInfoType" name="deleteInfoType" class="form-select" required>
				      		<option value="탈퇴">탈퇴</option>
				      		<option value="정지">정지</option>
				      	</select>
			      	</div>
			      	 	<div class="text-bg-secondary mb-2">관리자 이유(Comment) : </div>
			      		<textarea rows="5" name="deleteInfoComment" id="deleteInfoComment" placeholder="탈퇴/정지 사유를 입력해주세요" class="form-control mb-2"></textarea>
			      	<div>
			      	 <div class="text-bg-secondary mb-2">사용자 이유(Reason) : </div>
			      		<textarea rows="5" name="deleteInfoReason" id="deleteInfoReason" class="form-control"></textarea>
			      	</div>
		      	</form>
			    </div>
		      <div class="modal-footer">
		        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">닫기(저장안됨)</button>
		        <button type="button" onclick="adminMemberDeleteTableUpdateByMid()" class="btn btn-primary">수정</button>
		      </div>
		    </div>
		  </div>
		</div>
	...
	<div class="modal fade" id="chartModal" tabindex="-1" aria-labelledby="chartModalTitle" aria-hidden="true">
	  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl">
	    <div class="modal-content">
	      <div class="modal-header">
	        <h1 class="modal-title fs-5" id="chartModalTitle">모든 회원 파이차트(관리자)</h1>
	        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
	      </div>
	      <div class="modal-body">
