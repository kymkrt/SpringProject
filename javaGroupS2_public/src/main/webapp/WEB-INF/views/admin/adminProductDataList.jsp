...
<head>
	...
	<title>adminProductDataList.jsp</title>
	...
	<script type="text/javascript">
		...
		function allChecking() {
			let productCheckBtns = document.getElementsByName("productCheck");
			...
			if(allCheckBtn.checked){
				...
				for(let i=0; i<productCheckBtns.length; i++){
					productCheckBtns[i].checked = true;
				}
			}else{
				//memberCheckBtns = Array.from(memberCheckBtns);
				for(let i=0; i<productCheckBtns.length; i++){
					productCheckBtns[i].checked = false;
				}
			}
		}
		...
		//판매상태 변경
		 function productStatusChange(e) {
				let ans = confirm("선택한 상품의 판매상태를 변경하시겠습니까?");
				if(!ans) {
					location.reload();//새로고침
					return false;
				}
				//items[0] : 판매상태, items[1]: 고유번호			
				let items = e.value.split("/");
				
				$.ajax({
					type : "post", //get도 상관없음
					url : "${ctp}/admin/adminProductStatusChangeByIdx",
					data : {
						productStatus : items[0],
						idx : items[1]
					},
		 ...
		 function viewCheckOption(e) {//viewCheckOption_2
			    ...
			    if (!plusSelect) { // select 엘리먼트가 존재하지 않는 경우에만 생성
			        ...
			        plusSelect.addEventListener('change', (e) => {
			            ...
			            location.href = "${ctp}/admin/adminProductDataList?pageSize=${pageSize}&pag=${pag}&option="+optionValue+"&subOption="+ e.target.value;
			        });
			    ...
			    $.ajax({
			        type: "post",
			        url: "${ctp}/admin/adminProductOptionDataList",
			        data: {
			            optionData: e.value
			        },
			        dataType: "json",
		...
		$(document).ready(function() {
		    $('#chartSelect').change(function() {
		        ...
		        $.ajax({
		        	type: "post",
		        	url: "${ctp}/admin/adminProductDataChartCountList",
		        	data: {
...
<div id="container">
	<h2 class="text-center">전체 상품 데이터 목록(관리자)</h2>
	<div class="row mb-2">
		<div class="col-4">
		</div>
		<div class="col-4"></div>
		<div class="col-4 text-end">
			<button type="button" onclick="location.href='${ctp}/admin/adminProductDataInsert'" class="btn btn-info">
				상품 데이터 작성하기
			</button>
		</div>
	</div>
	...
	<table class="table table-hover table-bordered text-center align-middle">
		...
		<c:forEach var="vo" items="${vos}" varStatus="st">
			<tr> 
				<td>
					<c:if test="${sLevel == 0 }"><input type="checkbox" name="productCheck" id="productCheck${vo.idx}" value="${vo.idx}" class="form-check-input" /></c:if>
					<c:if test="${sLevel != 0 }"><input type="checkbox" name="productCheck" id="productCheck${vo.idx}" value="${vo.idx}" class="form-check-input" disabled /></c:if>
				</td>
				<td>
					<label for="productCheck${vo.idx}">
						${curScrStartNo}
					</label>
				</td>
				<td class="text-wrap">
					<a href="${ctp}/admin/adminProductDataDetailInfo?idx=${vo.idx}" data-bs-toggle="tooltip" data-bs-title="상품명:${vo.productName}<br/>상품데이터수정(관리자)" data-bs-html="true" class="link-info text-dark link-offset-2 link-opacity-75-hover link-underline-opacity-0 link-underline-opacity-100-hover">
						<b>${vo.productName}</b>
				...
				<td>
					<fmt:formatNumber value="${vo.deliveryCost}" pattern="#,###" />
				</td>
				<td class="text-wrap">
					${vo.productStock}
				</td>
				<td class="text-wrap">
					<select data-bs-toggle="tooltip" data-bs-title="판매상태변경" class="form-select" onchange="productStatusChange(this)">
						<option value="판매중/${vo.idx}" ${vo.productStatus=='판매중'?'selected':''}>판매중</option>
						<option value="임시등록/${vo.idx}" ${vo.productStatus=='임시등록'?'selected':''}>임시등록</option>
						<option value="판매중지/${vo.idx}" ${vo.productStatus=='판매중지'?'selected':''}>판매중지</option>
						<option value="재고없음/${vo.idx}" ${vo.productStatus=='재고없음'?'selected':''}>재고없음</option>
					</select>
				</td>
				<td class="text-wrap">
					${vo.productTag}
			...
			<div class="col-4">
				<div class="input-group">
					...
					<select id="chartSelect" name="chartSelect" class="form-select">
						...
						<option value="productCategory">카테고리별</option>
						<option value="productStatus">판매상태별</option>
					</select>
				</div>
			</div>
			<div class="col-4"></div>
  ...
  <div class="text-center">
	  <ul class="pagination justify-content-center">
		  <c:if test="${pageVO.pag > 1}"><li class="page-item"><a class="page-link text-secondary" href="adminProductDataList?pageSize=${pageVO.pageSize}&pag=1">첫페이지</a></li></c:if>
		  <c:if test="${pageVO.curBlock > 0}"><li class="page-item"><a class="page-link text-secondary" href="adminProductDataList?pageSize=${pageVO.pageSize}&pag=${(pageVO.curBlock-1)*pageVO.blockSize + 1}">이전블록</a></li></c:if>
		  <c:forEach var="i" begin="${(pageVO.curBlock*pageVO.blockSize)+1}" end="${(pageVO.curBlock*pageVO.blockSize) + pageVO.blockSize}" varStatus="st">
		    <c:if test="${i <= pageVO.totPage && i == pageVO.pag}"><li class="page-item active"><a class="page-link bg-secondary border-secondary" href="adminProductDataList?pageSize=${pageVO.pageSize}&pag=${i}">${i}</a></li></c:if>
		   	<!-- 배타적 처리 필요 -->
		    <c:if test="${i <= pageVO.totPage && i != pageVO.pag}"><li class="page-item"><a class="page-link text-secondary" href="adminProductDataList?pageSize=${pageVO.pageSize}&pag=${i}">${i}</a></li></c:if>
		  </c:forEach>
			  <c:if test="${pageVO.curBlock < pageVO.lastBlock}"><li class="page-item"><a class="page-link text-secondary" href="adminProductDataList?pageSize=${pageVO.pageSize}&pag=${(pageVO.curBlock+1)*pageVO.blockSize+1}">다음블록</a></li></c:if>
			  <c:if test="${pageVO.pag < pageVO.totPage}"><li class="page-item"><a class="page-link text-secondary" href="adminProductDataList?pageSize=${pageVO.pageSize}&pag=${pageVO.totPage}">마지막페이지</a></li></c:if>
	  	</ul>
		</div>
	  ...
	  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl">
	    <div class="modal-content">
	      <div class="modal-header">
	        <h1 class="modal-title fs-5" id="chartModalTitle">상품 데이터 파이차트(관리자)</h1>
