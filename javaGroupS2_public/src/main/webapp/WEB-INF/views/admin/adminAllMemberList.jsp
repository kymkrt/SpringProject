...
<head>
	...
	<title>adminAllMemberList.jsp</title>
	...
	<script type="text/javascript">
		...
		//탈퇴회원 복구시키기
		function adminMemberActivate(idx, mid) {
		 ...
		 function viewCheckOption(e, viewCheckOption_2) {
			    ...
			    if (!plusSelect) { // select 엘리먼트가 존재하지 않는 경우에만 생성
			        ...
			        plusSelect.addEventListener('change', (e) => {
			            ...
			            let optionValue = document.getElementById("viewCheckOption").value; //로그인 타입, 성별 등
			            location.href = "${ctp}/admin/adminAllMemberList?pageSize=${pageSize}&pag=${pag}&viewCheckOption=" + optionValue + "&viewCheckOption_2=" + e.target.value;
			        });
...
<div id="container">
	<h2 class="text-center">전체회원목록(관리자)</h2>
	<div class="row mb-2">
		<div class="col-4">
	...
	<table class="table table-hover table-bordered text-center align-middle">
		...
		<c:if test="${adminAllMembers == null || fn:length(adminAllMembers) == 0}">
			<tr>
				<td colspan="9">
					데이터가 없습니다.
				</td>
		...
		<c:forEach var="vo" items="${adminAllMembers}" varStatus="st">
			<tr class="${vo.userDel eq 'OK' ? 'table-warning' : ''}"> 
				...
				<td>
					<c:if test="${vo.userDel=='OK' && vo.mid != 'admin'}">
  ...
  <div class="text-center">
	  <ul class="pagination justify-content-center">
		  <c:if test="${pageVO.pag > 1}"><li class="page-item"><a class="page-link text-secondary" href="adminAllMemberList?level=${level}&pageSize=${pageVO.pageSize}&pag=1">첫페이지</a></li></c:if>
		  <c:if test="${pageVO.curBlock > 0}"><li class="page-item"><a class="page-link text-secondary" href="adminAllMemberList?level=${level}&pageSize=${pageVO.pageSize}&pag=${(pageVO.curBlock-1)*pageVO.blockSize + 1}">이전블록</a></li></c:if>
		  <c:forEach var="i" begin="${(pageVO.curBlock*pageVO.blockSize)+1}" end="${(pageVO.curBlock*pageVO.blockSize) + pageVO.blockSize}" varStatus="st">
		    <c:if test="${i <= pageVO.totPage && i == pageVO.pag}"><li class="page-item active"><a class="page-link bg-secondary border-secondary" href="adminAllMemberList?level=${level}&pageSize=${pageVO.pageSize}&pag=${i}">${i}</a></li></c:if>
		   	<!-- 배타적 처리 필요 -->
		    <c:if test="${i <= pageVO.totPage && i != pageVO.pag}"><li class="page-item"><a class="page-link text-secondary" href="adminAllMemberList?level=${level}&pageSize=${pageVO.pageSize}&pag=${i}">${i}</a></li></c:if>
		  </c:forEach>
			  <c:if test="${pageVO.curBlock < pageVO.lastBlock}"><li class="page-item"><a class="page-link text-secondary" href="adminAllMemberList?level=${level}&pageSize=${pageVO.pageSize}&pag=${(pageVO.curBlock+1)*pageVO.blockSize+1}">다음블록</a></li></c:if>
			  <c:if test="${pageVO.pag < pageVO.totPage}"><li class="page-item"><a class="page-link text-secondary" href="adminAllMemberList?level=${level}&pageSize=${pageVO.pageSize}&pag=${pageVO.totPage}">마지막페이지</a></li></c:if>
	  	</ul>
		</div>
  ...
  <div class="modal-dialog modal-dialog-centered">
	...
	<div class="modal fade" id="chartModal" tabindex="-1" aria-labelledby="chartModalTitle" aria-hidden="true">
	  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl">
	    <div class="modal-content">
	      <div class="modal-header">
	        <h1 class="modal-title fs-5" id="chartModalTitle">모든 회원 파이차트(관리자-<span style="color: crimson">탈퇴회원포함</span>)</h1>
	        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
	      </div>
	      <div class="modal-body">
